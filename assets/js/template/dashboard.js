...
var Dashboard = function () {
    ...
    var profitShare = function (at_last) {
        if ($('#m_chart_profit_share').length == 0) {
            return;
        }

        var url = window.location.origin + window.location.pathname;
        var url = url.replace('Welcome', '');
//        console.log(url);
        var var_labels = [];
        var var_datas = [];
        var set_data_pie = [];
...
//                console.log(val);
                ...
                var dashboard_row_jobs_div = '';
                var dashboard_pie_jobs_div = '';
                var set_data_pie = [];
                $.each(val.row, function (key, value) {
                    set_data_pie.push({value: value.count, className: 'custom', meta: {color: mUtil.getColor(value.colour)}});
                    var str = ' <div class="m-widget1__item">\
                                        <div class="row m-row--no-padding align-items-center">\
                                            <div class="col">\
                                                <h3 class="m-widget1__title">' + value.label + '</h3>\
                                                <span class="m-widget1__desc"> </span>\
                                            </div>\
                                            <div class="col m--align-right">\
                                                <span class="m-widget1__number m--font-' + value.colour + '">' + value.count + '  Job(s)</span>\
                                            </div>\
                                        </div>\
                                    </div>';
                    var str2 = ' <div class="m-widget14__legend">\
                                    <span class="m-widget14__legend-bullet m--bg-' + value.colour + '"></span>\
                                    <span class="m-widget14__legend-text">' + value.percent + ' % ' + value.label + '</span>\
                                </div>';
                    dashboard_row_jobs_div = dashboard_row_jobs_div + str;
                    dashboard_pie_jobs_div = dashboard_pie_jobs_div + str2;
                });
                $('#dashboard_row_jobs').html(dashboard_row_jobs_div);
                $('#dashboard_pie_jobs_div').html(dashboard_pie_jobs_div);
                $('#dashboard_pie_jobs_count').html(val.all_jobs);

                chart.data = {series: set_data_pie};
                chart.update(); 
                
//                console.log(set_data_pie);

//                var set_data_pie = [];
//                $.each(val.pie, function (key, value) {
//                    set_data_pie.push({label: value.Product_Code, value: value.count_movement});
//                    var str = ' <div class="m-widget1__item">\
//                                        <div class="row m-row--no-padding align-items-center">\
//                                            <div class="col">\
//                                                <h3 class="m-widget1__title">' + value.label + '</h3>\
...
//                set_data_pie.push({label: 'Other', value: val.other_products});
//                var div_canvas = '  <div class="m-widget16__visual" style="margin-top:15px;">\
//                                        <div id="m_chart_top_movement" style="height: 350px"></div>\
//                                    </div>';
//                div_top_movement_products = div_top_movement_products + div_canvas;
////                $('#a_top_movement_products').html('Last ' + at_last + ' days');
//                $('#div_top_movement_products').html(div_top_movement_products);
//
//                Morris.Donut({
//                    element: 'm_chart_top_movement',
//                    data: set_data_pie,
////                    labelColor: '#a7a7c2',
//                    labelColor: '#34bfa3',
//                    colors: [
...
//            console.log(process);
//            console.log(params);
//            bandwidthChart1(params);
            switch (function_name) {
                case 'profitShare':
                    profitShare(param1, param2);
                    break;
                default:
                    console.log('default');
            }
